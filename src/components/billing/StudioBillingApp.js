...
const StudioBillingApp = () => {
	...
	const getFilteredCustomers = () => {
		...
		return sorted.filter((customer) => customer.name.toLowerCase().includes(searchLower) || (customer.phone && customer.phone.includes(searchLower)));
	};
	...
	const updateAdditionalItem = (index, field, value) => {
		const updated = additionalItems.map((item, i) => (i === index ? { ...item, [field]: value } : item));
	...
	const updateEditAdditionalItem = (index, field, value) => {
		const updated = editAdditionalItems.map((item, i) => (i === index ? { ...item, [field]: value } : item));
	...
	return (
		<div className="max-w-6xl mx-auto p-6 bg-white dark:bg-gray-800 min-h-screen" dir="rtl">
			<h1 className="text-3xl font-bold text-center mb-8 text-gray-800 dark:text-gray-100">🎙️ מערכת חיוב אולפן הקלטות</h1>
			...
			<div className="grid lg:grid-cols-3 gap-8">
				...
				<div className="lg:col-span-2 space-y-6">
					<div className="bg-blue-50 dark:bg-gray-700 p-6 rounded-lg">
						...
						<div className="mb-4">
								...
								{showCustomerDropdown && customerSearchTerm && (
									<div className="absolute z-10 w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg max-h-60 overflow-auto mt-1">
										...
										:	getFilteredCustomers().map((customer) => (
												...
												>
													{customer.name}
													{customer.phone && <span className="text-xs text-gray-500 dark:text-gray-400 mr-2">{customer.phone}</span>}
												</div>
											))
										}
									</div>
							...
							{selectedClient && (
								<div className="mt-2 text-sm text-blue-600 dark:text-blue-400">
									חוב לא שולם: ₪{selectedClient.total_unpaid || 0} | חשבונות: {selectedClient.unpaid_invoices || 0} לא שולמו,{" "}
									{(selectedClient.total_invoices || 0) - (selectedClient.unpaid_invoices || 0)} שולמו
								</div>
							)}
						</div>
						...
						{/* Timer Section */}
						<div
							className={`mb-4 p-4 rounded-md border transition-all duration-300 ${
								isTimerRunning ? "bg-green-50 dark:bg-green-900/30 border-green-300 dark:border-green-700 animate-pulse" : "bg-white dark:bg-gray-600 border-gray-300 dark:border-gray-500"
							}`}
						>
							<div className="flex items-center justify-between mb-3">
								...
								<div className={`text-2xl font-mono ${isTimerRunning ? "text-green-700 dark:text-green-300 font-bold" : "dark:text-gray-200"}`}>{formatTime(elapsedTime)}</div>
							</div>
							<div className="flex gap-2">
								<button onClick={startTimer} disabled={isTimerRunning} className="flex items-center gap-1 px-3 py-2 bg-green-500 text-white rounded-md disabled:opacity-50 cursor-pointer">
									<Play size={16} /> התחל
								</button>
								<button onClick={pauseTimer} disabled={!isTimerRunning} className="flex items-center gap-1 px-3 py-2 bg-yellow-500 text-white rounded-md disabled:opacity-50 cursor-pointer">
									<Pause size={16} /> השהה
								</button>
								<button onClick={stopTimer} className="flex items-center gap-1 px-3 py-2 bg-red-500 text-white rounded-md cursor-pointer">
									<Square size={16} /> עצור
								</button>
							</div>
						</div>
						...
						<div className="grid grid-cols-2 gap-4 mb-4">
							<div>
								...
								<input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-600 dark:border-gray-500 dark:text-white" />
							</div>
							<div>
								<label className="block text-sm font-medium mb-2 dark:text-gray-200">שעת סיום</label>
								<input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-600 dark:border-gray-500 dark:text-white" />
							</div>
						</div>

						<div className="mb-4">
							<label className="block text-sm font-medium mb-2 dark:text-gray-200">תאריך</label>
							<input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-600 dark:border-gray-500 dark:text-white" />
						</div>
					...
					<div className="bg-green-50 dark:bg-gray-700 p-6 rounded-lg">
						<div className="flex justify-between items-center mb-4">
							...
							<button onClick={addAdditionalItem} className="flex items-center gap-1 px-3 py-2 bg-green-500 text-white rounded-md cursor-pointer">
								<Plus size={16} /> הוסף
							</button>
						</div>
						{additionalItems.map((item, index) => (
							<div key={index} className="flex gap-2 mb-2">
								...
								<input
									...
									min="0"
								/>
								<button onClick={() => removeAdditionalItem(index)} className="p-2 bg-red-500 text-white rounded-md cursor-pointer">
									<Trash2 size={16} />
								</button>
							</div>
						))}
				...
				<div className="space-y-6">
					...
					<div className="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg">
						...
						<div className="space-y-2 text-sm dark:text-gray-300">
							<div>
								<strong>משך הסשן:</strong> {formatHours(calculateDuration())} שעות
							</div>
							<div>
								<strong>מחיר בסיס:</strong> ₪{(calculateDuration() * hourlyRate).toFixed(2)}
							</div>
							<div>
								<strong>אחרי הנחות:</strong> ₪{(calculateDuration() * hourlyRate * (1 - (parseFloat(percentDiscount) || 0) / 100) - (parseFloat(shekelDiscount) || 0)).toFixed(2)}
							</div>
							<div>
								<strong>תוספות:</strong> ₪{additionalItems.reduce((sum, item) => sum + (parseFloat(item.price) || 0), 0).toFixed(2)}
							</div>
					...
					{generatedInvoice && (
						<div className="bg-white dark:bg-gray-700 p-6 rounded-lg border-2 border-blue-200 dark:border-blue-700">
							...
							<div className="whitespace-pre-line text-sm bg-gray-50 dark:bg-gray-800 p-4 rounded-md border dark:border-gray-600 font-mono dark:text-gray-200">{generatedInvoice}</div>
						</div>
					)}
					...
					{paymentLink && (
						<div className="bg-green-50 dark:bg-gray-700 p-6 rounded-lg border-2 border-green-200 dark:border-green-700">
							...
							<div className="text-sm bg-white dark:bg-gray-800 p-4 rounded-md border dark:border-gray-600 break-all dark:text-gray-200">{paymentLink}</div>
						</div>
					)}
									...
									<div key={invoice.id} className={`flex justify-between items-center text-sm p-3 rounded ${invoice.paid ? "bg-green-100 dark:bg-green-900/30" : "bg-red-100 dark:bg-red-900/30"}`}>
										...
										<div className="flex items-center gap-2">
											...
											<div className="flex gap-1">
												...
												<button onClick={() => startEditInvoice(invoice)} className="p-1 bg-blue-500 text-white rounded hover:bg-blue-600 cursor-pointer" title="ערוך">
													✏️
												</button>
												<button onClick={() => deleteInvoice(invoice.id)} className="p-1 bg-red-500 text-white rounded hover:bg-red-600 cursor-pointer" title="מחק">
													🗑️
												</button>
											</div>
										</div>
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" dir="rtl">
					<div className="bg-white dark:bg-gray-800 p-8 rounded-lg max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
						<div className="flex justify-between items-center mb-6">
							...
							<button onClick={() => setShowDebtorsModal(false)} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 cursor-pointer">
								<X size={24} />
							</button>
						</div>

						{debtors.length === 0 ?
							<div className="text-center py-8 text-gray-500 dark:text-gray-400">🎉 אין חובות פתוחים!</div>
						:	<>
								<div className="mb-6 p-4 bg-purple-100 dark:bg-purple-900/30 rounded-lg">
									<div className="text-center">
										<div className="text-2xl font-bold text-purple-800 dark:text-purple-200">₪{debtors.reduce((sum, d) => sum + parseFloat(d.total_unpaid), 0).toFixed(2)}</div>
										<div className="text-sm text-purple-600 dark:text-purple-300">סה"כ חובות | {debtors.length} חייבים</div>
									</div>
								</div>
										...
										<div key={debtor.client_id} className="border dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
											<div className="flex justify-between items-start mb-2">
												<div>
													...
													<div className="text-sm text-gray-600 dark:text-gray-400">{debtor.unpaid_count} חשבונות לא שולמו</div>
												</div>
												<div className="text-left">
													<div className="text-2xl font-bold text-red-600 dark:text-red-400">₪{parseFloat(debtor.total_unpaid).toFixed(2)}</div>
												</div>
											</div>
											...
											<div className="mt-3 flex gap-2 flex-wrap">
												...
												<button onClick={() => markAllDebtorInvoicesPaid(debtor.client_id)} className="px-3 py-1 bg-emerald-500 text-white text-sm rounded hover:bg-emerald-600 cursor-pointer">
													✅ סמן הכל כשולם
												</button>
												<button
													onClick={() => {
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" dir="rtl">
					<div className="bg-white dark:bg-gray-800 p-8 rounded-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
						<div className="flex justify-between items-center mb-6">
							...
							<button onClick={() => setShowEditModal(false)} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 cursor-pointer">
								<X size={24} />
							</button>
						</div>
						...
						<div className="space-y-4">
							...
							<div>
								<label className="block text-sm font-medium mb-2 dark:text-gray-200">תאריך</label>
								<input type="date" value={editDate} onChange={(e) => setEditDate(e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-600 dark:border-gray-500 dark:text-white" />
							</div>
							...
							<div>
								<div className="flex justify-between items-center mb-2">
									...
									<button onClick={addEditAdditionalItem} className="px-3 py-1 bg-green-500 text-white rounded text-sm cursor-pointer">
										+ הוסף
									</button>
								</div>
								{editAdditionalItems.map((item, index) => (
									<div key={index} className="flex gap-2 mb-2">
										...
										<input
											...
											min="0"
										/>
										<button onClick={() => removeEditAdditionalItem(index)} className="p-2 bg-red-500 text-white rounded cursor-pointer">
											🗑️
										</button>
									</div>
								))}
